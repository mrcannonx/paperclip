...
🛑 **THERE IS NO READ-BY-ID. `GET /v1/memory/<id>` returns `{"detail":"Method Not Allowed"}`** — and that is VALID JSON, so a verification loop that json-parses the reply and prints on a successful parse reports **OK for every row while having read nothing**. (Only `DELETE /v1/memory/<id>` exists.) To confirm a write landed, `search` with a distinctive phrase from the memory's own text. 🛑 **`GET /v1/memory` (the LIST) returns ONLY rows with NO `project_slug`, and `limit` tops out at 100 — `limit=101` or more silently returns `[]`.** `?category=` and `?tag=` filter INSIDE that unslugged subset, so a lane-slugged row (every `memory-store --slug` write) is invisible to the list even when the write landed — matching an id prefix against the list reads a good write as MISSING. Verified 2026-09-12: two fresh slugged rows absent from `?category=feedback`, `?tag=wallet-explains` returned 0, `GET /v1/apps/<slug>/memory` answers "Invalid or missing credentials" with the factory key. Search sees everything.
